...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let RegisterUser = (e) => {
    ...
    if (!validateEmail(emailRegister.value.trim())) {
      ...
      return;
    } else if (!validateField(companyName.value.trim())) {
    ...
    createUserWithEmailAndPassword(
      ...
      .then((credentails) => {
        set(ref(db, "CompanyAuthList/" + credentails.user.uid), {
  ...
  let SignInUser = (e) => {
    ...
    signInWithEmailAndPassword(
      ...
      .then((credentails) => {
        ...
        get(child(dbref, "CompanyAuthList/" + credentails.user.uid)).then(
          (snapShot) => {
            ...
            if (snapShot.exists) {
              ...
              sessionStorage.setItem("user-state", "company");
